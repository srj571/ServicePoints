...
public class TC37_VerifyStopFulfillmentAndStartFulfillmentTest extends BaseClass {
	...
	public void verifyStopFulfillmentAndRequoteTest() throws InterruptedException, IOException, AWTException {
		...
		if (driver.getPageSource().contains("No more product quotations")) {
			logger.info(
					"Verification of visiblity of product after Stop fullifilling in Quotations Client tab in Agent side is Successfull.");
			Assert.assertTrue(true);
			Thread.sleep(2000);
		} else {
			logger.info(
					"Verification of visiblity of product after Stop fullifilling in Products tab in Agent side is failed.");
			Assert.assertTrue(false);
			Thread.sleep(2000);
		}
		...
		if (driver.getPageSource().contains("No more product quotations")) {
			logger.info(
					"Verification of visiblity of product after Stop fullifilling in Products tab in Agent side is Successfull.");
			Assert.assertTrue(true);
			Thread.sleep(2000);
		} else {
			logger.info(
